...
def main_worker_stage2(args,log_s1_name,log_s2_name):
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            # 对IR聚类
            data_ir_cam = [x for x in dataset_ir.train if x[2] == ir_cam_id]
            print('==> Create pseudo labels for camera{} unlabeled IR data'.format(ir_cam_id))
            cluster_loader_ir = get_test_loader(dataset_ir, args.height, args.width,
                                            64, args.workers, testset=sorted(data_ir_cam))
            features_ir, labels_ir = extract_features(model, cluster_loader_ir, print_freq=50,mode=2)
            del cluster_loader_ir
            features_ir = torch.cat([features_ir[f].unsqueeze(0) for f, _, _ in sorted(data_ir_cam)], 0)
            ...
            #index_f = []
            for i, ((fname, _, cid), label) in enumerate(zip(sorted(data_ir_cam), pseudo_labels_ir)):
                if label != -1:
                    pseudo_labeled_dataset_ir.append((fname, label.item(), cid))
                #index_f.append(fname)
            print('==> Statistics for IR epoch {} cam{}: {} clusters'.format(epoch, ir_cam_id, num_cluster_ir))
                
        '''_,topk_cam = torch.topk(torch.mm(features_ir_, features_ir_.T), 200)
        ...
        memory_ir_cam = ClusterMemory(768*cls_token_num, num_cluster_ir, temp=args.temp, momentum=args.momentum0, use_hard=args.use_hard)
        memory_ir_cam.features = F.normalize(cluster_features_ir, dim=1).cuda()
        trainer.memory = memory_ir_cam
        ...
        trainer.train(epoch,train_loader_ir, optimizer, print_freq=10, train_iters=len(train_loader_ir), mode=2) ###############
        del train_loader_ir

            
        
        if epoch >= 10:
            silhouette = metrics.silhouette_score(rerank_dist_ir, pseudo_labels_ir, metric='precomputed')
            ...
            if is_best:
                ...
                np.save('/scratch/chenjun3/liulekai/PGM-ReID-main/logs/sysu_cam/'+str(ir_cam_id)+'.npy', pseudo_labeled_dataset_ir)
